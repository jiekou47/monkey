...
namespace driver
{
    internal class API
    {
        ...
        public static void Yuandetected_frompic()
        {
            Debug.WriteLine("检测圆形!");
            using (var capture = new VideoCapture(0))
            {

                Mat src = new Mat();
                capture.Read(src);
                //src = Cv2.ImRead("yuan.jpg", ImreadModes.Color);
                if (src.Empty())
                {
                ...
                Cv2.Canny(blurred, cannyImage, 50, 200);
                // 霍夫圆变换  
                CircleSegment[] circles = Cv2.HoughCircles(cannyImage, HoughModes.Gradient, 1, blurred.Rows / 8,
                    param1: 200, param2: roi, minRadius: 0, maxRadius: 0);
                Debug.WriteLine(circles.Length);

                // 绘制圆形  
                //foreach (var circle in circles)
                //{
                //    Point center = new Point(Convert.ToInt32(circle.Center.X), Convert.ToInt32(circle.Center.Y));
                //    int radius = Convert.ToInt32(circle.Radius);
                //    Cv2.Circle(src, center, 3, Scalar.Red, -1, LineTypes.AntiAlias);
                //    Cv2.Circle(src, center, radius, Scalar.Green, 3, LineTypes.AntiAlias);
                //}
                if (circles.Length > 0)
                {
                    Point center = new Point(Convert.ToInt32(circles[0].Center.X), Convert.ToInt32(circles[0].Center.Y));
                    int radius = Convert.ToInt32(circles[0].Radius);
                    Cv2.Circle(roiMat, center, 3, Scalar.Red, -1, LineTypes.AntiAlias);
                    Cv2.Circle(roiMat, center, radius, Scalar.Green, 3, LineTypes.AntiAlias);
                    save_pic(src);
                }
                ...
                //// 显示结果  
                //
                Cv2.ImShow("Detected Circles", cannyImage);
                Cv2.WaitKey(0);

                Cv2.ImShow("Detected Circles", roiMat);
                Cv2.WaitKey(0);
        ...
        public static void save_pic(Mat temp)
        {
